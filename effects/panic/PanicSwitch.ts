...
export class PanicSwitch extends Effect {
    ...
    public override async start(): Promise<void> {
        ...
        let warning: number = 10;

        let durationInMS: number = duration * 1000;
        let warningInMS: number = warning * 1000;

        Util.playSound("effects/panic/alarm.mp3");

        let panicStr: string = "/announce ThisIsFine elmoFire " + this.triggerData.user + 
        ...
        let warningStr: string = "/announce WARNING!! ONLY " + warning +
            " SECONDS REMAIN!!";
        app.twitch.bot.say(warningStr, true);
        
        await Util.sleep(warningInMS);

        let orderStr: string = "/announce Order has been restored. Sound Effects," + 
